...
import { getMonthlyHistory, getLifetimeStats, getRankHistory, getCurrentMonthData, getAnalyticsData, getBadgeInfo } from '@/lib/analyticsUtils'
...
export function AnalyticsTab() {
      ...
      <section className="card">
        ...
        <div style={{
          ...
          <p style={{color: 'var(--text-muted)', marginBottom: '24px'}}>
            Calculate your team incentives in Bulk mode to start tracking your performance history.
          </p>
          <div style={{
  ...
  const getProjections = () => {
    ...
    const projectedAchievement = (projectedSales / currentMonth.teamTarget) * 100
  ...
  const chartData = history.slice(-6).map(m => ({
        ...
        <div style={{padding: '24px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', marginBottom: '32px', border: '2px solid var(--border-color)'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px'}}>
            <div style={{padding: '16px', background: 'var(--bg-primary)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)'}}>
              <div style={{fontSize: '12px', color: 'var(--text-muted)', marginBottom: '4px'}}>Current Tier</div>
              <div style={{fontSize: '20px', fontWeight: '700', color: 'var(--text-primary)'}}>{currentMonth.tier}</div>
              <div style={{fontSize: '12px', color: 'var(--text-secondary)', marginTop: '4px'}}>{currentMonth.tierRate}% rate</div>
            </div>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{fontSize: '12px', color: 'var(--text-muted)', marginTop: '2px'}}>
            Tracking {history.length} months • Stored locally in your browser
          </div>
        </div>
        <div style={{display: 'flex', gap: '8px'}}>
          <button 
            className="btn btn-secondary"
            onClick={() => exportAnalyticsToPDF(history, lifetimeStats, rankHistory)}
          >
            📕 Export PDF
          </button>
          <button 
